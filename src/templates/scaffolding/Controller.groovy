...
class ${className}Controller {
	...
	String SEARCH_COUNT = '${domainClass.propertyName}.count'
	String SEARCH_QUERY_STRING = '${domainClass.propertyName}.queryString'
	String SEARCH_QUERY_PARAMS = '${domainClass.propertyName}.queryParams'
	
	// the delete, save and update actions only accept POST requests
	static allowedMethods = [save: "POST", update: "POST", delete: "POST"];

	String modelName = message(code:'${domainClass.propertyName}')
	...
	def index = {
		clearPageSession()
		render(view:VIEW_LIST, model:[instance:new ${className}(), instanceTotal:0])
	}
	...
	def list = {
		if(session[SEARCH_QUERY_STRING]){
			...
			def ${domainClass.propertyName}Count = ${className}.count();
			session[SEARCH_COUNT] = ${domainClass.propertyName}Count
			def ${domainClass.propertyName}Instance = new ${className}(params)
			render(view:VIEW_LIST,model:[instanceList: ${className}.list(params), instanceTotal: ${domainClass.propertyName}Count, instance: ${domainClass.propertyName}Instance])
		}
	}

	def create = {
		clearPageSession()
		def instance = new ${className}()
	...
	def search = {
		...
		def queryString = 'from ${className} as d where 1=1 '
		def queryParams = []
...
//			}
			try{
				def queryBean = CrudHelper.parseFindCondition(${className}, params)
		...
		def results = ${className}.findAll(queryString,queryParams,paginate)
		def resultsCount
		if(session[SEARCH_QUERY_PARAMS] != queryParams){
			resultsCount = ${className}.executeQuery('select count(*) '+queryString,queryParams)[0]
		...
		def ${domainClass.propertyName}Instance = new ${className}(params)
		render(view:VIEW_LIST,model:[instanceList:results,instanceTotal:resultsCount, instance: ${domainClass.propertyName}Instance])
		
	}
	
	def export = {
		if(!${className}.metaClass.hasProperty(${className},'exportFields')){
			forward(action:'search',params:params)
			return;
		}
		if(session[SEARCH_QUERY_STRING]){
			def writer = CrudHelper.exportDataToWriterStream(${className}, response, params, 
