...
class UserConfigController {
	...
	String VIEW_LIST = '/cfg/userConfig/list'
	String VIEW_CREATE = '/cfg/userConfig/create'
	String VIEW_EDIT = '/cfg/userConfig/edit'
	String VIEW_SHOW = '/cfg/userConfig/show'
	
	String SEARCH_COUNT = 'userConfig.count'
	String SEARCH_QUERY_STRING = 'userConfig.queryString'
	String SEARCH_QUERY_PARAMS = 'userConfig.queryParams'
	
	// the delete, save and update actions only accept POST requests
	static allowedMethods = [save: "POST", update: "POST", delete: "POST"];

	String modelName = message(code:'userConfig')
	...
	def index = {
		clearPageSession()
		render(view:VIEW_LIST, model:[instance:new UserConfig(), instanceTotal:0])
	}
	...
	def list = {
		if(session[SEARCH_QUERY_STRING]){
			...
			def userConfigCount = UserConfig.count();
			session[SEARCH_COUNT] = userConfigCount
			def instance = new UserConfig(params)
			render(view:VIEW_LIST,model:[instanceList: UserConfig.list(params), instanceTotal: userConfigCount, instance: instance])
		}
	}

	def create = {
		clearPageSession()
		def instance = new UserConfig()
	...
	def show = {
		def instance = UserConfig.get(params.id)
		if (!instance) {
	...
	def bulkDelete = {
		...
		if(params.ids){
			...
			for(id in params.ids){
				def instance = UserConfig.get(id)
				if (instance) {
					try {
						instance.delete()
						CrudHelper.decrementResultCount(session, SEARCH_COUNT)
	...
	def search = {
		...
		def queryString = 'from UserConfig as d where 1=1 '
		def queryParams = []
		...
		if(params.retainCondition){
			...
			try{
				def queryBean = CrudHelper.parseFindCondition(UserConfig, params)
		...
		def results = UserConfig.findAll(queryString,queryParams,paginate)
		def resultsCount
		if(session[SEARCH_QUERY_PARAMS] != queryParams){
			resultsCount = UserConfig.executeQuery('select count(*) '+queryString,queryParams)[0]
	...
	def export = {
		if(!UserConfig.metaClass.hasProperty(UserConfig,'exportFields')){
			forward(action:'search',params:params)
			return;
		}
		if(session[SEARCH_QUERY_STRING]){
			def writer = CrudHelper.exportDataToWriterStream(UserConfig, response, params,
